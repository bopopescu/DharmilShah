...
contract Exchange {
  function assert(bool assertion)public view {
    if (!assertion) revert();
  }
  ...
  function setInactivityReleasePeriod(uint256 expiry) onlyAdmin public returns (bool success) {
    if (expiry > 1000000) revert();
    inactivityReleasePeriod = expiry;
    return true;
  }

  constructor(address feeAccount_)public payable{
    owner = msg.sender;
  ...
  function depositToken(address token , uint256 amount)public payable{
    ...
    if (!Token(token).transferFrom(msg.sender, this, amount)) revert("error 1");
  ...
  function withdraw(address token, uint256 amount) public returns (bool ) {
    if (safeSub(block.number, lastActiveTransaction[msg.sender]) < inactivityReleasePeriod) revert("lastActiveTransaction error:");
    if (tokens[token][msg.sender] < amount) revert("token error :");
    tokens[token][msg.sender] = safeSub(tokens[token][msg.sender], amount);
    if (token == address(0)) {
      if (!msg.sender.send(amount)) revert("msg_sender error :");
    } else {
      if (!Token(token).transfer(msg.sender, amount)) revert("Withdraw transfer error : ");
    }
  ...
  function adminWithdraw(address token, uint256 amount, address user, uint256 nonce, uint8 v, bytes32 r, bytes32 s, uint256 feeWithdrawal) onlyAdmin public returns (bool ) {
    ...
    if (withdrawn[hash]) revert("hash error");
    withdrawn[hash] = true;
    if (ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", hash)), v, r, s) != user) revert("ecrecover error: ");
    if (feeWithdrawal > 50 finney) feeWithdrawal = 50 finney;
    if (tokens[token][user] < amount) revert("token error:");
    tokens[token][user] = safeSub(tokens[token][user], amount);
    tokens[token][feeAccount] = safeAdd(tokens[token][feeAccount], safeMul(feeWithdrawal, amount) / 1 ether);
    amount = safeMul((1 ether - feeWithdrawal), amount) / 1 ether;
    if (token == address(0)) {
      if (!user.send(amount)) revert("user error :");
    } else {
      if (!Token(token).transfer(user, amount)) revert("transfer error :");
    }
  ...
  function trade(uint256[8] tradeValues, address[4] tradeAddresses, uint8[2] v, bytes32[4] rs) onlyAdmin public returns (bool) {
    ...
    /* tradeValues
     ...
     tradeAddressses
       ...
       [3] taker
     */
    if (invalidOrder[tradeAddresses[2]] > tradeValues[3]) revert("invalid order error :");  
    ...
    if (ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", orderHash)), v[0], rs[0], rs[1]) != tradeAddresses[2]) revert("ecrecover 1 error :");
    bytes32 tradeHash = keccak256(abi.encodePacked(orderHash, tradeValues[4], tradeAddresses[3], tradeValues[5])); 
    if (ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", tradeHash)), v[1], rs[2], rs[3]) != tradeAddresses[3]) revert("ecrecover 2 error :");
    if (traded[tradeHash]) revert("traded hash error: ");
    traded[tradeHash] = true;
    if (tradeValues[6] > 100 finney) tradeValues[6] = 100 finney;
    if (tradeValues[7] > 100 finney) tradeValues[7] = 100 finney;
    if (safeAdd(orderFills[orderHash], tradeValues[4]) > tradeValues[0]) revert("orderFills error :");
    if (tokens[tradeAddresses[0]][tradeAddresses[3]] < tradeValues[4]) revert("token error :");
    if (tokens[tradeAddresses[1]][tradeAddresses[2]] < (safeMul(tradeValues[1], tradeValues[4]) / tradeValues[0])) revert("token error 2 :");
